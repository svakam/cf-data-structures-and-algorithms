...
const count = (target, input) => {
  ...
  let overallCounter = input.reduce((overallCount, subarray) => {
    if (subarray === []) {
      return 0;
    }
    let subCounter = subarray.reduce((subCount, value) => {
      if (target === value) {
        subCount = subCount + 1;
      }
      return subCount;
    }, 0);
    overallCount = overallCount + subCounter;
    return overallCount;
  }, 0);
...
const totalSum = (input) => {
  let overallSummer = input.reduce((sum, subarray) => {
    let subSummer = subarray.reduce((subsum, value) => {
      subsum += value;
      return subsum;
    }, 0);
    sum += subSummer;
...
let findShortest = (data) => {
  let shortest = 20000;
  let shortestChar = data.filter(char => {
    if (parseInt(char.height, 10) < shortest) {
      shortest = parseInt(char.height, 10);
      return char;
    }
  }).filter(char => {
    if (parseInt(char.height, 10) === shortest) {
